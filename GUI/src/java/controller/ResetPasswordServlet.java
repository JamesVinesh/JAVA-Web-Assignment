...
public class ResetPasswordServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection conn = DriverManager.getConnection(host, user, pwd)) {
            String sql = "UPDATE CUSTOMER SET CUSTPASSWORD = ? WHERE CUSTEMAIL = ?";
            ...
            if (updated > 0) {
                ...
                response.getWriter().println("<script>alert('Password reset successful!'); window.location='login.jsp';</script>");
            } else {
